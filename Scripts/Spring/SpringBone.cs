...
namespace GraduationProject
{
	public class SpringBone : MonoBehaviour
	{
		...
		private SpringCollider[] colliders;
		...
		public void SetSpringCollider (SpringCollider[] cols)
		{
			SpringCollider[] tempColliders = cols;
			int[] tempArray = new int[tempColliders.Length];
			...
			{
				if ((cols[i].transform.position - transform.position).magnitude < (cols[i].radius+radius)*5f)
				{
			...
			SpringCollider[] colls = new SpringCollider[tempIndex];
			for (int i=0; i<tempIndex; i++)
			{
				colls[i] = tempColliders[tempArray[i]];
			}
